    ...
    stages {
        ...
        stage('Deliver') {
            steps {
                ...
                sh "./mvnw package -Pprod verify jib:build -Djib.to.image=\$CONTAINER_REGISTRY/\$IMAGE_REPOSITORY:\$env.BUILD_NUMBER"
                // script {
        ...
        stage('Deploy') {
            ...
            steps {
                ...
                sh "az webapp create --resource-group \"\$AZURE_RESOURCE_GROUP\" --plan \"\$AZURE_APP_SERVICE_PLAN\" --name \"\$AZURE_APP_NAME\" --deployment-container-image-name \"\$CONTAINER_REGISTRY/\$IMAGE_REPOSITORY:\$env.BUILD_NUMBER\""
                sh 'az logout'
            }
        }
    }
